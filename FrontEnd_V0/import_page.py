...
class ImportPage(QtWidgets.QWidget):
    ...
    def start_layering(self) -> None:
        ...
        # Build backend-compatible task payload (Header / Item / Quality)
        get = edited.get
        payload = {
            "Header": {
                "fileId": get("文件编号", ""),
                "deliveryDate": get("交付日期", ""),
            },
            "Item": {
                "sku": get("货号", ""),
                "partName": get("部件", ""),
                "colorway": get("配色", ""),
                "sizeRanges": self._split_list(get("码段", "")),
                "quantities": self._normalize_quantities(get("数量", "")),
                "fabricMaterial": get("面料材质", ""),
                "fabricThickness": get("面料厚度", ""),
                "baseLayerThickness": self._split_list(get("打底层厚", "")),
                "inkLayerThickness": get("油墨层厚", ""),
                "designGraphic": get("图案设计", ""),
            },
            "Quality": {
                "colorFastness": {
                    "dryRubbing": get("干摩擦", ""),
                    "wetRubbing": get("湿摩擦", ""),
                    "waterFastness": get("水牢度", ""),
                },
                "foldResistanceCount": self._to_int(get("耐折次数", 0)),
            },
        }
    ...
    def _on_layering_success(self, data: dict) -> None:
        # Store result and navigate to layering page
    ...
    def _to_int(self, value, default=0) -> int:
        """
        Parse an int, returning a default on failure.
        """
        try:
            return int(value)
        except Exception:
    ...
    # grouped private helpers (parsing / utils)
